...
int main(int argc, char * argv[]) {
	...
	if (rank == 0) { // This code is run by a single process
		...
		MPI_Status status;
		for(i = 0; i < nProcesses - 1; ++i) {
			MPI_Recv(v+i, 1, MPI_INT, i + 1, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		}
		...
		for(int i = 0; i < nProcesses - rank; ++i) {
			int recvValue;
			MPI_Recv(&recvValue, 1, MPI_INT, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
			if(savedValue == -1) {
				savedValue = recvValue;
			} else if(recvValue < savedValue) {
				MPI_Send(&savedValue, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
				savedValue = recvValue;
			} else if(recvValue >= savedValue) {
				MPI_Send(&recvValue, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
			}
		}

		MPI_Send(&savedValue, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
	}
